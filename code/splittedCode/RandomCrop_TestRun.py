...
from helpfuncs import camel2snake, listify, accuracy_multi_label, macro_metric_multi_label, recall_score_multi_label, precision_score_multi_label, accuracy_score_multi_label
from transformations import FreqMask, TimeMask, target_to_one_hot, DownmixMono
from databunch import DataBunch, ClassSpecificImageFolder, WholeDataset, AudioFolder
from callbacks import StatsCallback, SaveCheckpointCallback, UseCacheCallback, TestInferenceCallback
from learner import Runner, Learner, create_BirdNET, create_ResNet50, ddp_create_BirdNET, ddp_create_ResNet50
...
def main(args):
    ...
    au_transform = transforms.Compose(
        ...
        #transforms.Resize((128,2560)), #(64,384)
        transforms.RandomCrop(size=(128,345), pad_if_needed=True),
        #FreqMask(max_mask_size_F=5, num_masks = 5),
        #TimeMask(max_mask_len_T=5, num_masks = 5),
        transforms.Normalize((0.5), (0.5))])

    au_transform_val = transforms.Compose(
        [DownmixMono(),
        spectro,
        #transforms.Grayscale(num_output_channels=1),
        #transforms.Resize((128,2560)), #(64,384)
    ...
    small_ds = "/lustre/scratch2/ws/0/s4030475-ml_birds_project/bird_data/spectrograms/xeno-canto-data/splitfolder_audio/"
    ...
    train_ds = AudioFolder(root=train_path, dropped_classes=dropped_classes, transform=au_transform, target_transform=target_t)
    ...
    train_dl = DataLoader(train_ds, batch_size=256, shuffle=(train_sampler is None),
            num_workers=10, pin_memory=False, sampler=train_sampler, drop_last=True)
    ...
    val_ds = AudioFolder(root=val_path, dropped_classes=dropped_classes, transform=au_transform_val, target_transform=target_t)
    ...
    val_dl = DataLoader(val_ds, batch_size=256, shuffle=True,
            num_workers=10, pin_memory=False, sampler=val_sampler, drop_last=True)
    ...
    learn = Learner(*ddp_create_BirdNET(LEN_CLASSES, lr=0.01, args=args), nn.BCEWithLogitsLoss(), databunch)
    #learn = Learner(*ddp_create_ResNet50(LEN_CLASSES, lr=0.01, pretrained=False, args=args), nn.BCEWithLogitsLoss(), databunch)

    if args.rank == 0:
        print("metrics")
        metric_list = [accuracy_score_multi_label, recall_score_multi_label, precision_score_multi_label]
        stats_cbf = partial(StatsCallback, metric_list, newDir)
        checkpoint_cbf = partial(SaveCheckpointCallback, newDir)
        test_inf_cbf = partial(TestInferenceCallback, newDir)
        run = Runner(cb_funcs=[stats_cbf, checkpoint_cbf], rank = args.rank)
    else:
        print("Runner()")
        run = Runner(rank = args.rank, distributed=args.distributed)
